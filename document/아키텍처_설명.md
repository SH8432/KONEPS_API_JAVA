...
| 파일 | 역할 |
|------|------|
| `NaraApiConfig` | API 기본 URL, 인증 키(`PersonalAuthKey`) 제공 |

**왜 분리했는지**

- **한 곳에서만 설정 변경**: URL·키가 바뀌어도 `NaraApiConfig`만 수정하면 됩니다.
- 서비스 키는 `NaraApiConfig.PersonalAuthKey` 상수에 하드코딩되어 있어, 키 변경 시 이 클래스만 수정하면 됩니다.
...
| 파일 | 역할 |
|------|------|
| `NaraApiService` | Client 호출 + Parser 사용 + 그리드용 변환·필터(최소 배정예산·검색어·업무구분)·여러 페이지(병렬) 수집 |

**왜 분리했는지**

- **UI와 API 사이의 “애플리케이션 로직” 한 곳에 모음**:  
  “한 번 호출해서 그리드로 보여주기”, “최소 금액·검색어·업무구분 필터를 걸고 여러 페이지를 병렬로 다 가져오기” 같은 흐름은 모두 Service에서만 처리합니다.
...
추가로, 여러 페이지를 조회할 때는 **API 문서의 권장 동시 요청 수(3~5개)와 초당 호출 제한(30 tps)** 을 고려해, 1페이지 이후는 최대 5페이지까지 병렬로 호출하고 배치 간 짧은 지연을 두는 방식으로 구현했습니다. 이렇게 해서 전체 조회 시간을 줄이면서도 공공 API 사용 가이드를 지키도록 했습니다.
...
- **메인 클래스**: `com.softbase.nara.ui.NaraGetSwing`
- **API 키**: `NaraApiConfig.PersonalAuthKey` 상수에 정의되어 있습니다.
